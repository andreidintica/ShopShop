...
class ProductController
{
    ...
    public function addProduct()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!empty($name) && !empty($description) && !empty($price) && !empty($stock) && !empty($idType)) {
                $type = $this->typeModel->getTypeById((int)$idType);
                
                if ($type === null) {
                    $_SESSION['message'] = 'Erreur sur le type.';
                } else {
                    $product = new Product(null, $name, $description, (float)$price, (int)$stock, $type);
